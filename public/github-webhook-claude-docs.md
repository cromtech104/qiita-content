...
tags:
  ...
  - Claude
private: false
updated_at: '2026-06-15T17:11:53+09:00'
...
この2ステップをコードの自動修正に応用した場合の実装（JSONパースの堅牢化含む）は[別の記事](https://qiita.com/items/86ac82d3807990af0b7a)で書いています。
...
ドキュメントの種類が増えてくると、LLMの呼び出しが積み重なって数分かかることがある。Webhookを受け取るLambdaで同期処理すると余裕でタイムアウトする（Lambdaのタイムアウトはデフォルト3秒、最大15分だが、GitHubがWebhookのタイムアウトとみなす時間はもっと短い）。
...
本記事で解説した仕組みは、GitHubリポジトリのドキュメントをAIで自動生成するSaaS [RepoCarta](https://repocarta.jp/) の実装として実際に動いています。同じ課題を感じている方はぜひ触ってみてください。

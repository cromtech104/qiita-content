...
GitHubへのpushをトリガーに、リポジトリのドキュメントをClaudeで自動更新するSaaSを個人で作っている。「pushしたらClaudeが読んで更新してくれる」というコンセプト自体はシンプルなんだけど、実際に作るといくつかハマった。
...
GitHubのWebhookを扱う系の実装、署名検証と冪等性だけは最初からきちんとやっておくと後で助かる。コンテキスト設計はリポジトリの規模次第で全然変わってくるのでそこは様子を見ながら。
